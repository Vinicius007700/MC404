...
void pack(int *encoded, InstData data)
{
    switch (data.type)
    {
    case R:
        bit_operations(data.funct7, 7, 25, encoded);
        bit_operations(data.rs2, 5, 20, encoded);
        bit_operations(data.rs1, 5, 15, encoded);
        bit_operations(data.funct3, 3, 12, encoded);
        bit_operations(data.rd, 5, 7, encoded);
        bit_operations(data.opcode, 7, 0, encoded);
        return;
    case I:
        bit_operations(data.imm, 12, 20, encoded);
        bit_operations(data.rs1, 5, 15, encoded);
        bit_operations(data.funct3, 3, 12, encoded);
        bit_operations(data.rd, 5, 7, encoded);
        bit_operations(data.opcode, 7, 0, encoded);
        return;
    case S:
        bit_operations(data.imm >> 5, 7, 25, encoded);
        bit_operations(data.rs2, 5, 20, encoded);
        bit_operations(data.rs1, 5, 15, encoded);
        bit_operations(data.funct3, 3, 12, encoded);
        bit_operations(data.imm, 5, 7, encoded);
        bit_operations(data.opcode, 7, 0, encoded);
        return;

    case B:
        bit_operations(data.imm >> 12, 1, 31, encoded); // imm[12]
        bit_operations(data.imm >> 5, 6, 25, encoded);  // imm[10:5]
        bit_operations(data.rs2, 5, 20, encoded);
        bit_operations(data.rs1, 5, 15, encoded);
        bit_operations(data.funct3, 3, 12, encoded);
        bit_operations(data.imm >> 1, 4, 8, encoded);   // imm[4:1]
        bit_operations(data.imm >> 11, 1, 7, encoded);  // imm[11]
        bit_operations(data.opcode, 7, 0, encoded);
        return;
    case U:
        bit_operations(data.imm, 20, 12, encoded); //imm[31:12]
        bit_operations(data.rd, 5, 7, encoded);
        bit_operations(data.opcode, 7, 0, encoded);
        return;
    case J:
        bit_operations(data.imm >> 20, 1, 31, encoded); // imm[20]
        bit_operations(data.imm >> 1, 10, 21, encoded); // imm[10:1]
        bit_operations(data.imm >> 11, 1, 20, encoded); // imm[11]
        bit_operations(data.imm >> 12, 8, 12, encoded); // imm[19:12]
        bit_operations(data.rd, 5, 7, encoded);
        bit_operations(data.opcode, 7, 0, encoded);
        return;
    }
